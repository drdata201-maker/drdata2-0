...
export function FreelanceReportsPage() {
  const { t } = useLanguage();
  ...
  const fetchReports = async () => {
    const { data } = await supabase.from("reports").select("*").eq("user_type", "freelance").order("created_at", { ascending: false });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!session) return;
      const { error } = await supabase.from("reports").insert({ user_id: session.user.id, title: title.trim(), type, status: "draft", user_type: "freelance", content: { notes: content.trim() } });
