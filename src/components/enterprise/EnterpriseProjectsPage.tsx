...
export function EnterpriseProjectsPage() {
  ...
  const fetchProjects = async () => {
    const { data } = await supabase.from("projects").select("*").eq("user_type", "enterprise").order("created_at", { ascending: false });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("enterprise.sidebar.projects")}</h1>
          <p className="mt-1 text-muted-foreground">{t("enterprise.projects.desc")}</p>
        </div>
        <Button onClick={() => navigate("/dashboard/enterprise/new-analysis")}><Plus className="mr-2 h-4 w-4" /> {t("pme.projects.create")}</Button>
      </div>
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center py-12 text-center">
              ...
              <p className="text-muted-foreground">{t("enterprise.recentProjects.empty")}</p>
              <Button variant="outline" className="mt-4" onClick={() => navigate("/dashboard/enterprise/new-analysis")}><Plus className="mr-2 h-4 w-4" /> {t("pme.projects.create")}</Button>
            </div>
          ) : (
