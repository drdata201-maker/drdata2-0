...
export function EnterpriseNewAnalysisPage() {
  const { t } = useLanguage();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!session) return;
      const { error } = await supabase.from("analyses").insert({ user_id: session.user.id, title: title.trim(), type, status: "pending", user_type: "enterprise", results: { description: description.trim() } });
      if (error) throw error;
      toast.success(t("pme.newAnalysis.success"));
      navigate("/dashboard/enterprise/projects");
  ...
  const analysisTypes = [
    ...
    { value: "strategic", label: t("enterprise.analysisType.strategic") },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">{t("enterprise.sidebar.newAnalysis")}</h1>
        <p className="mt-1 text-muted-foreground">{t("enterprise.newAnalysis.desc")}</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("pme.newAnalysis.titlePlaceholder")} />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground">{t("pme.newAnalysis.type")}</label>
            <Select value={type} onValueChange={setType}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{analysisTypes.map((at) => (<SelectItem key={at.value} value={at.value}>{at.label}</SelectItem>))}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
