...
import { Building2, Camera, Sun, Moon, Monitor, LogOut, Trash2, Lock } from "lucide-react";
...
export function EnterpriseSettingsView({
  companyName,
  userEmail,
  userCountry,
  industry,
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      {/* Company Profile */}
      <section>
        <h2 className="mb-4 text-lg font-semibold text-foreground">{t("enterprise.settings.companyProfile")}</h2>
        <div className="rounded-xl border border-border bg-card p-6 space-y-6">
          <div className="flex items-center gap-4">
            <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-primary text-xl font-bold text-primary-foreground">
              {companyName?.charAt(0)?.toUpperCase() || <Building2 className="h-6 w-6" />}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <Label className="text-muted-foreground">{t("enterprise.settings.companyName")}</Label>
              <Input defaultValue={companyName} className="mt-1" />
            </div>
            <div>
            ...
            <div>
              <Label className="text-muted-foreground">{t("enterprise.settings.industry")}</Label>
              <Input defaultValue={industry} className="mt-1" />
            </div>
            <div>
              <Label className="text-muted-foreground">{t("enterprise.settings.companySize")}</Label>
              <Input defaultValue={companySize} className="mt-1" />
            </div>
            <div>
