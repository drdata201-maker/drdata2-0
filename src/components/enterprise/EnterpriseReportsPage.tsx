...
interface ReportRow { id: string; title: string; type: string; status: string; created_at: string; }
...
export function EnterpriseReportsPage() {
  ...
  const fetchReports = async () => {
    const { data } = await supabase.from("reports").select("*").eq("user_type", "enterprise").order("created_at", { ascending: false });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!session) return;
      await supabase.from("reports").insert({ user_id: session.user.id, title: title.trim(), type, status: "draft", user_type: "enterprise", content: { notes: content.trim() } });
      toast.success(t("pme.reports.created"));
      setTitle(""); setContent(""); setType("executive"); setOpen(false); fetchReports();
    } catch { toast.error(t("pme.newAnalysis.error")); }
    finally { setCreating(false); }
  };

  const handleDelete = async (id: string) => { await supabase.from("reports").delete().eq("id", id); setReports((p) => p.filter((r) => r.id !== id)); toast.success(t("pme.reports.deleted")); };

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("enterprise.sidebar.reports")}</h1>
          <p className="mt-1 text-muted-foreground">{t("enterprise.reports.desc")}</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" /> {t("pme.reports.new")}</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <Select value={type} onValueChange={setType}>
                ...
                <SelectContent>
                  <SelectItem value="executive">{t("enterprise.reports.executive")}</SelectItem>
                  <SelectItem value="department">{t("enterprise.reports.department")}</SelectItem>
                  <SelectItem value="financial">{t("pme.analysisType.financial")}</SelectItem>
                  <SelectItem value="strategic">{t("enterprise.analysisType.strategic")}</SelectItem>
                </SelectContent>
              </Select>
                ...
                <TableBody>
                  {reports.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => handleDelete(r.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
